...
function computeDailyStrain(activities, wellnessByDate, stepsBaselineByDate){
  ...
  for(const date of allDates){
    ...
    if(!hasActivityOnDate.has(date) && w && w.steps){
      ...
      raw += stepsToRawStrain(w.steps, baseline);
    }
...
function computeResults(recs, activities){
  const hrvStats = rollingStats(recs, 'hrv', NEW_METHOD_CUTOFF);
  ...
  const results = recs.map((r,i)=>{
    ...
    return {...r, recovery: built.recovery, tsb: tsbSeries[i], fatigueScore: fatigueScoreSeries[i], fatigueRatio: fatigueRatioSeries[i]};
  });
